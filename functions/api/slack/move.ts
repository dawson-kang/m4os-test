...
async function getDoc(projectId: string, token: string, id: string): Promise<{ channel: string; ts: string } | null> {
  ...
  return {
    channel: data.fields?.channel?.stringValue ?? '',
    ts:      data.fields?.ts?.stringValue      ?? '',
  };
}
...
export async function onRequestPost(context: any) {
  ...
  try {
    ...
    // Sync Slack reaction (fire-and-forget — failures do not affect response)
    if (doc?.channel && doc?.ts) {
      const action = targetStatus === 'archived' ? 'add' : 'remove';
      fetch(`https://slack.com/api/reactions.${action}`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${env.SLACK_BOT_TOKEN}`,
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({ channel: doc.channel, timestamp: doc.ts, name: 'm4_archive' }),
      }).catch(() => {});
    }
